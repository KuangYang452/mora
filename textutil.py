...
延迟导入原本是潜在循环依赖，见 1.0_RELEASE_PLAN.md §5.3）。
...
def strip_time_prefix(text: str) -> str:
    """剥离开头连续的时间标注前缀（[刚刚] / （2分钟前）等），无则原样返回。

    模型偶会把上下文消息里的时间标注（含历史遗留的全角括号样式）当成
    自己的发言格式复制进输出；这里是硬清洗兜底，不依赖指令。
    只匹配「刚刚 / N分钟前 / N小时前 / N天前 / N个月前」的括号标注，
    不触碰台词内容（含（歪头凑近）这类动作描写）。
    """
    if not text:
        return text
...
def strip_paren_annotations(text: str) -> str:
    ...
    只匹配完整配对的圆括号（最多剥 3 层嵌套），不触碰方括号相对时间
    标注（由 strip_time_prefix 处理）；清洗后压缩多余空白，不碰换行。
    """
    if not text:
        return text
    for _ in range(3):

...
```json
{
  ...
  "goal": "角色目标（可选）：长期动机/目标，字符串或字符串数组（数组按行渲染）",
  "methodology": "方法论（可选）：达成目标的工作方法，字符串或字符串数组（数组按行渲染）",
  "relationships": {
    "<关系对象>": "关系描述"
  },
  "abilities": {
...
- 【你的身份与世界观】由程序按固定模板渲染（身份块 + 世界观块 + 扩展块），
  不删减内容。`goal` 与 `methodology` **不渲染进身份段**：目标在激活指令尾部
  贴近输出（近因效应）、方法论在系统提示词【方法论】静态段——二者分置投放、
  互不重复（见下）。
- `goal`（可选）在**激活指令尾部贴近输出单独一行**（「你的目标：…」；正文唯一
  来源 = 契约 `goal`）——与内容模式裸标记同为约定 7 的「关键规则贴近输出重复
  一次」（模型有忽略长设定的偏置，目标埋在长文里容易「知道但没执行」）。
- `methodology`（可选）渲染为系统提示词**【方法论】静态段**（紧跟【记忆与回忆】
  之后、行为协议之前）：达成目标的工作方法常驻系统提示词，不随回合变化，参与
  静态缓存前缀。

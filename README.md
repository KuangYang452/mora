...
4. **提示词组装顺序：静态在前、动态在后**。system 提示词只含纯静态段（身份/
   ...
   （【当前状态】——好感度数值隐去只留等级与内心想法、【{USER_REFERENCE}正在…】游戏环境
...
6. **LLM 输出全部走原生函数调用，权限低于硬编码**。模型通过 OpenAI 兼容 API 的
   **原生 function calling** 通道输出：**台词也是工具**（调用 `say` 工具说出），
   ...
   兜底（模型未走工具时降级处理，不崩溃）。
7. **提示词吝啬（工程级约定）**。提示词每一字同时影响成本与服从度：篇幅越大，
   token 成本越高、服从度越低。本条约束工程师的编辑行为，对抗两种惯性：
   「出问题就**新增禁令**」的无序膨胀；「**只对当前段落就地打补丁**」、不回头
   考虑复用。实现约束按优先级：① **收回既有许可**（删掉一条既有「允许」条款
   达成新约束，等效新增「禁止」、篇幅零增）；② **引用式复用**既有段落或既有
   格式，不另起新段；③ 确需新增的，**贴合现成格式 + 最小必要**，不写防御性
   措辞（「绝对不要」类预防性词汇），不枚举输出通道（通道已被架构封死，见
   约定 6）。去重：**普通规则单一事实来源**，同一规则不得在上下文两处出现；
   仅当规则存在**已知模型偏置、且只有贴近输出位置才生效**时，才视为**关键
   规则**，允许按 recency 在贴近输出处**重复一次**。实例：内容模式（NSFW/SFW）
   共两处——【输出红线】块一行（`- 不允许回避R18内容。台词尺度不得低于
   所读文本：直白词照用、不代称不省略、不降级为暗示。`，贴合红线格式
   「- 主体：约束。」，不重复模式含义）+ 激活指令首行裸标记
   `内容模式:NSFW`（紧邻【本回合指令】、贴近输出位置——模型有安全化倾向，
   需输出前重复强调）。篇幅预算（约定 7 的机器可执行形态，锁定量化基线
   防开发迭代回潮）：`llm_prompt.prompt_budget()` 单一预算表 + selftest
   断言 + `debug.py --prompt` 逐段打印，超限即压缩或带理由改预算。
...
- **NSFW**：许可成人词汇与直白的性描写；涉及成人话题时**不允许回避**——
  对方话题是什么尺度，就按什么尺度回应。红线行额外约束**词汇对等**：所读
  文本用直白词时输出照用，不得代称、省略或降级为暗示（实测模型会把原文
  「阳物」在 think 收敛成「巨物」、say 再收敛成「大东西」，双层各降一档）。
...
不重复模式含义）：`- 不允许回避R18内容。台词尺度不得低于所读文本：
直白词照用、不代称不省略、不降级为暗示。`（SFW 为
`- 不允许输出R18内容。`）；**激活指令首行裸标记**
`内容模式:NSFW`（紧邻【本回合指令】、贴近输出位置——模型有安全化倾向，
